...
const MotionIn1DTest = () => {
  ...
  useEffect(() => {
    const fetchQuestions = async () => {
      try {
        const response = await axios.get(`${BASE_URL}/api/questions?topic=Motion%20in%201D`);
  ...
  return (
    <div className={`test-container ${darkMode ? "dark" : ""}`}>
      ...
      <h1 className={`test-title ${darkMode ? "dark-text" : ""}`}>Motion in 1D Test</h1>
